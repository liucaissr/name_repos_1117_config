...
def job_config = [
        job: [
                name: "name_repos_1117_Pipeline_develop"
        ],
        git: [
                branch: "develop"
        ]
...
def customLib = library identifier: 'name_repos_1117_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/ORGANISATION/name_repos_1117_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.name_repos_1117.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "name_repos_1117_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
